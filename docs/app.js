...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: IDFM GTFS (Île-de-France Mobilités)' }));
  ...
  const PALETTE = [
    ...
    // IDFM official metro line colors (uppercase — exactly as the pipeline
    // emits them from routes.txt; a color missing here loses its station
    // discs, so keep the list in sync with data/out/stops.geojson)
    ['#0055C8', '#002f6e'],
    ['#00643C', '#003721'],
    ['#640082', '#370048'],
    ['#6E491E', '#3d2811'],
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: IDFM (Île-de-France Mobilités)';
      ...
      a.download = `paris-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `paris-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the T13 tram-train and a few radial buses reach
  // far into the couronne, so fitting the data bbox would open on countryside
  // with the city as a small core. Open on central Paris instead.
  map.jumpTo({ center: [2.3488, 48.8534], zoom: 11.3 });
}

...
// Paris: ONE IDFM feed covers the whole Île-de-France (~2000 lines), so the
// map's scope is a PRECOMPUTED allowlist (data/scope.json, pipeline/scope.mjs):
// buses (3) whose stops sit mostly within the city + petite couronne, all
...
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/paris.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['38']),
  feeds: [
    { tag: 'idfm', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['3'],
      skipRoute: (r) => !SCOPE_BUS.has(r.route_id) },
  ],
}];
// The rail slot splits in TWO cfgs sharing mode 'tram': street trams and the
// metro must NOT share a graph — metro shapes often pass closer to the tram
// tracks running above the tunnel than to the subway axis (13–48 m at Dristor
...
if (tramAll || tramSel.length) MODES.push({
  ...
  // railway=rail from its RER C days — without it the drawn line stopped at
  // Épinay and 437 observations had no candidate track. Yards and sidings
  // stay out.
  mode: 'tram', label: 'trams', osmFile: 'data/osm/paris-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  railExtra: (e) => e.tags?.railway === 'rail' && !e.tags?.service,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'idfm', dir: 'data/gtfs', mapKey: (sn) => sn, routeTypes: ['0'],
      skipRoute: (r) => !SCOPE_TRAM.has(r.route_id) },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  // the feed names metro lines with bare numbers — prefixed to M1…M14 so the
  // keys are unambiguous next to bus numbers and trigger the metro treatment
  mode: 'tram', label: 'metro', osmFile: 'data/osm/paris-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'idfm', dir: 'data/gtfs',
      // nine metro numbers are also bus numbers in the IDFM scope, so the key
      // keeps an M; the roundel in the street says 1, and so does the map —
